...
Write-Host "Laster alle drivere fra FileRepository..."

$DriverPath = "X:\Windows\System32\DriverStore\FileRepository"

# Last inn alle .inf-filer automatisk
Get-ChildItem -Path $DriverPath -Recurse -Filter *.inf | ForEach-Object {
    try {
        drvload $_.FullName | Out-Null
        Write-Host "Loaded driver: $($_.Name)"
    }
    catch {
        Write-Host "Failed to load driver: $($_.Name)"
    }
}

Write-Host "Alle tilgjengelige drivere forsøkt lastet – fortsetter..."
Start-Sleep -Seconds 3


Write-Host "Waiting for network adapter to get an IP address..."
$maxAttempts = 60
$attempt = 0
$gotIP = $false
...
while ($attempt -lt $maxAttempts) {
    ...
    $ipOutput = ipconfig | Out-String
    
    if ($ipOutput -match 'IPv4[^:]+:\s*(\d{1,3}(\.\d{1,3}){3})') {
        $foundIP = $Matches[1]

        if ($foundIP -ne '0.0.0.0' -and $foundIP -notlike '169.254.*') {
            Write-Host "IP address obtained: $foundIP"
            $gotIP = $true
            break
        }
    }
    ...
    $attempt++
    Write-Host "No IP yet. Retrying in 5 seconds... (attempt $attempt/$maxAttempts)"
    Start-Sleep -Seconds 5
}

if (-not $gotIP) {
    Write-Error "No network connection could be established."
    exit 1
}
...
# Start SMB client (important in WinPE)
net start lanmanworkstation
Start-Sleep -Seconds 3

$NetworkShare = '\\192.168.1.50\ffushare'
$DriveLetter  = 'Z:'
...
net use $DriveLetter $NetworkShare $Password /user:$UserString /persistent:no

# Check result
if ($LASTEXITCODE -ne 0) {
    Write-Error "Failed to map $DriveLetter to $NetworkShare"
    exit 1
}

# Verify mapped drive
if (-not (Test-Path "$DriveLetter\")) {
    Write-Error "Mapped drive $DriveLetter is not accessible."
    exit 1
}
...
$LogFileName = 'ScriptLog_' + $SerialNumber + '.txt'
...
If ($APFileToInstall) {
    ...
    try {
        Rename-Item -Path "W:\Windows\Provisioning\Autopilot\$APFileName" -NewName 'AutoPilotConfigurationFile.json'
